...
#+begin_src emacs-lisp
  ...
  (defun my/leader-key-mode--get-keymap-for-lsp-project-and-major-mode ()
    ...
    (let* ((lsp-root (lsp-workspace-root))
	   (major-mode-keymap-alist
	    (my/assoc-default-insert my/leader-key-mode--active-lsp-root-alist lsp-root (cons major-mode (list nil))))
	   (keymap
	    (my/assoc-default-insert (cdr major-mode-keymap-alist) major-mode (my/create-lsp-code-map))))
      (message "lsp root is %s" lsp-root)
      keymap))

  (defun my/leader-key-mode--filter-code-keymap (cmd)
    "return the default code keymap if lsp-mode is off in this buffer, otherwise return the lsp code keymap. CMD is unused."
    (message "filtering!")
    (if lsp-mode
	(progn
  ...
  (define-minor-mode my/leader-key-mode
    ...
    :keymap
    (let ((new-prefix-keymap (make-sparse-keymap)))
      (define-key new-prefix-keymap
		  (kbd "c") `(menu-item "" nil :filter ,(lambda (cmd) (if (buffer-modified-p) my/modified-test-keymap my/unmodified-test-keymap))))
      (which-key-add-keymap-based-replacements new-prefix-keymap
	"." '("find file" . find-file)
	"/" '("fuzzy search" . consult-ripgrep)
	"g" '("git" . magit)
	":" '("mode only command" . consult-mode-command)
	...
	"t" `("Toggle" . ,my/toggle-keymap)
	"c" "code")
      (let ((new-leader-keymap (make-sparse-keymap)))
	(define-key new-leader-keymap
		    (kbd "C-SPC") new-prefix-keymap)
	new-leader-keymap))
    :lighter "my-leader"
    :interactive t
  ...
  (define-key my/leader-key-mode-map (kbd "M")
	      `(menu-item "" nil :filter ,(lambda (cmd) (if (buffer-modified-p) my/modified-test-keymap my/unmodified-test-keymap))))
  ;; (define-key my/leader-key-mode-map
  ;; 	    "a" `(menu-item "" nil :filter ,(lambda (cmd) (if (buffer-modified-p) my/modified-test-keymap my/unmodified-test-keymap))))
  ;; (which-key-add-keymap-based-replacements my/leader-key-mode-map
  ;;   "." '("find file" . find-file)
  ;;   "/" '("fuzzy search" . consult-ripgrep)
  ;;   "g" '("git" . magit)
  ;;   ":" '("mode only command" . consult-mode-command)
  ...
  ;;   "i" `("Insert" . ,my/insert-keymap)
  ;;   "t" `("Toggle" . ,my/toggle-keymap)
  ;;   "c" `("Code" . menu-item "" nil :filter ,(lambda (cmd) (if (buffer-modified-p) my/modified-test-keymap my/unmodified-test-keymap)))
  ;;   )
  ;; (defvar-keymap test-keymap
  ;;   "a" `(menu-item "" nil :filter ,(lambda (cmd) (if (buffer-modified-p) my/modified-test-keymap my/unmodified-test-keymap)))
  ;;   )
