...
#+begin_src emacs-lisp
  ...
  (defvar-keymap my/project-keymap
    ...
    "s" #'projectile-find-related-file
    ) 

  (defvar-keymap my/file-keymap
    "p" #'my/open-personal-config-dir
  ...
  (defvar-keymap my/not-wanted-lsp-code-keymap
    "d"
    (lambda ()
      (if lsp-mode
...
#+begin_src emacs-lisp
  ...
  ;; (evil-define-keymap my/leader-key-keymap
  ;;   "My custom Evil keymap."
  ;;   :mode evil-mode
  ;;   :local t
  ;;   (kbd " ") my/leader-keymap
  ;;   )

  ;; (defvar-keymap my/leader-key-minor-mode-map)
  ;;   (define-minor-mode my/leader-key-minor-mode
  ;; 	"A mode for my custom keybindings."
  ;; 	:keymap my/leader-key-minor-mode-map
  ;; 	)
  ;;     (add-hook 'my/leader-key-minor-mode-hook (lambda () (make-local-variable 'my/leader-key-minor-mode-map)))
  ;;     (my/leader-key-minor-mode t)
  ...
  (defun my/create-lsp-code-map ()
	...
	(keymap-set my/lsp-code-keymap "p" #'lsp-describe-thing-at-point)
	(keymap-set my/lsp-code-keymap "a" #'lsp-execute-code-action)
	(keymap-set my/lsp-code-keymap "=" #'lsp-format-buffer)
	(keymap-set my/lsp-code-keymap "h" #'lsp-document-highlight)

	(when (lsp-feature? "textDocument/definition")
	  (keymap-set my/lsp-code-keymap "d" #'lsp-find-definition))

	(when (lsp-feature? "textDocument/references")
	  (keymap-set my/lsp-code-keymap "r" #'lsp-find-references))

	(when (lsp-feature? "textDocument/implementation")
	  (keymap-set my/lsp-code-keymap "i" #'lsp-find-implementation))

	(when (lsp-feature? "textDocument/declaration")
	  (keymap-set my/lsp-code-keymap "d" #'lsp-find-declaration))

	(when (lsp-feature? "textDocument/typeDefinition")
	  (keymap-set my/lsp-code-keymap "t" #'lsp-find-type-definition))
	...
	(when (or (lsp-can-execute-command? "java.navigate.resolveTypeHierarchy")
	  ...
	  (keymap-set my/lsp-code-keymap "T" #'lsp-treemacs-type-hierarchy))

	(when (and (functionp 'lsp-treemacs-call-hierarchy)
		   (lsp-feature? "textDocument/callHierarchy"))
	  (keymap-set my/lsp-code-keymap "c" #'lsp-treemacs-call-hierarchy))

	(when (lsp-feature? "textDocument/rename")
	  (keymap-set my/lsp-code-keymap "R" #'lsp-rename))

	;; Session keybindings
	(keymap-set my/lsp-code-keymap "S l" #'lsp-workspace-logs-show)
	(keymap-set my/lsp-code-keymap "S d" #'lsp-describe-session)
	(keymap-set my/lsp-code-keymap "S s" #'lsp-shutdown-workspace)

	;; Workspace folders keybindings
	(keymap-set my/lsp-code-keymap "F a" #'lsp-workspace-folders-add)
	(keymap-set my/lsp-code-keymap "F r" #'lsp-workspace-folders-remove)
	(keymap-set my/lsp-code-keymap "F o" #'lsp-workspace-folders-open)

	;; Toggle lsp features keybindings
	(keymap-set my/lsp-code-keymap "T l" #'lsp-lens-mode)
	(keymap-set my/lsp-code-keymap "T h" #'lsp-headerline-breadcrumb-mode)
	(keymap-set my/lsp-code-keymap "T a" #'lsp-modeline-code-actions-mode)
	(keymap-set my/lsp-code-keymap "T o" #'lsp-modeline-diagnostics-mode))
      my/lsp-code-keymap))
