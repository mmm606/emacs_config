...
#+end_src
*** be able to differentiate between C-m and RET with <C-m>
note that I am using this answer to differentiate between C-m and RET, this will only work in gui emacs: https://emacs.stackexchange.com/questions/20240/how-to-distinguish-c-m-from-return
Use (kbd "<C-m>") to differentiate "C-m" from ret
#+begin_src emacs-lisp
...
#+end_src
** custom functions needed for config
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
    ...
    (use-package org-roam
      :custom
      (org-roam-directory (concat (my/personal-config-dir) "/roam-notes/"))
...
#+end_src
*** lsp-mode
Note that the doc popup in the minibuffer is from eldoc mode
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (use-package lsp-ui
    ;; :hook (lsp-mode . lsp-ui-mode) ;; lsp-mode is hooked into by default
    :custom
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap my/surround-keymap)
  (which-key-add-keymap-based-replacements my/surround-keymap
    "\"" `("\"..\"" . ,(lambda () (interactive) (insert-pair ?\" ?\")))
    "'" `("'..'" . ,(lambda () (interactive) (insert-pair ?\' ?\')))
    "{" `("{..}" . ,(lambda () (interactive) (insert-pair ?{ ?})))
    "[" `("[..]" . ,(lambda () (interactive) (insert-pair ?[ ?])))
    "(" '("(..)" . insert-parentheses)
...
#+begin_src emacs-lisp
    ...
    (which-key-add-keymap-based-replacements my/project-keymap
      ...
      "C-s" '("save proj buffers" . projectile-save-project-buffers))
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap my/note-keymap)
  (which-key-add-keymap-based-replacements my/note-keymap
    ;; "n" '("notes" . org-roam-buffer-toggle)
    "f" '("find notes" . org-roam-node-find)
    "c" '("capture note" . org-roam-capture)
    "l" '("toggle backlinks" . org-roam-buffer-toggle)
    "i" '("insert roam link" . org-roam-node-insert))
...
#+begin_src emacs-lisp
	...
	(when (bound-and-true-p lsp-mode)
	  ...
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    ...
	    "l" '("highlight" . lsp-document-highlight)
	    "h" '("hover" . lsp-ui-doc-glance)
	    )
	  ...
	  (let ((my/session-lsp-code-keymap (define-keymap)))
	    (which-key-add-keymap-based-replacements my/session-lsp-code-keymap
	      ...
	      ;; toggle
	      "L" '("toggle code lens" . lsp-lens-mode)
	      "H" '("toggle breadcrumb" . lsp-headerline-breadcrumb-mode)
	      "A" '("toggle modeline code actions" . lsp-modeline-code-actions-mode)
	      "D" '("toggle modeline diagnostics" . lsp-modeline-diagnostics-mode))
	    (which-key-add-keymap-based-replacements my/lsp-code-keymap "L" `("Lsp Session" . ,my/session-lsp-code-keymap)))
...
#+begin_src emacs-lisp
  ...
  (defvar-keymap my/org-local-leader-keymap)
  (which-key-add-keymap-based-replacements my/org-local-leader-keymap
    "s" '("org source block" . org-insert-structure-template))

  (push (cons 'org-mode my/org-local-leader-keymap) my/major-mode-local-leader-keymap-alist)
...
#+begin_src emacs-lisp
  ...
  (which-key-add-keymap-based-replacements my/leader-keymap
    ...
    "r" `("Rings and Regs" . ,my/rings-and-regs-keymap)
    "e" `("Edit" . ,my/edit-keymap)
    "d" `("Diagnostics" . ,my/error-keymap)
    "n" `("notes" . ,my/note-keymap)
    "t" `("Toggle" . ,my/toggle-keymap))

  (defvar my/filtered-local-leader-keymap-menu-item
    '(menu-item "local-leader" nil :filter my/filter-local-leader-keymap))
  ...
  (define-minor-mode my/leader-key-mode
    ...
    (let ((new-minor-mode-keymap (make-sparse-keymap)))
      ...
      (define-key new-minor-mode-keymap
		  (kbd "<C-m>") my/filtered-local-leader-keymap-menu-item)
  ...
  (evil-define-minor-mode-key 'normal 'my/leader-key-mode
    (kbd "m") my/filtered-local-leader-keymap-menu-item)
...
Also affects dirvish, so good for dirvish or dired
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (use-package dirvish
    ...
    (dirvish-override-dired-mode) ;; dirvish instead of dired
    ...
    (("C-c f" . dirvish-fd)
     ...
     ("M-j" . dirvish-fd-jump)
     ("<mouse-1>" . 'dirvish-subtree-toggle-or-open) ;; left click
     ("<mouse-2>" . 'dired-mouse-find-file-other-window) ;; right click
     ("<mouse-3>" . 'dired-mouse-find-file))) ;; middle click
