...
#+begin_src emacs-lisp
  ...
  (defun my/call-in-order (alist-of-func-keys-and-arg-values)
  ...
  ;;   (defvar-keymap rune/lsp-code-keymap
  ...
  ;;   (defvar-keymap rune/leader-keymap
  ;;     "c" rune/code-keymap
  ;;     )

  ;;   (global-set-key (kbd "C-M-z") rune/leader-keymap)

  ;;   (add-hook lsp-mode-hook (lambda () (which-key-add-keymap-based-replacements rune/leader-keymap "c" `("code" . ,rune/lsp-code-keymap))))

  ;;   (rune/leader-keys
  ;;     "c" `(,rune/overall-code-keymap :which-key "code")
  ;;     )
  ...
  ;;     "fp" '(my/open-personal-config-dir :which-key "personal config")
  ...
  ;;   (setq my/help-map (copy-keymap help-map))

  ;;   (rune/leader-keys "h" '(:keymap my/help-map :which-key "help"))
...
#+begin_src emacs-lisp
  ...
  (which-key-add-keymap-based-replacements my/toggle-keymap
    "t" '("consult theme" . consult-theme)
    "s" '("hydra text scale" . hydra-text-scale/body)
    "d" '("ediff" . ediff))

  ;; Insert keybindings
  (defvar-keymap my/insert-keymap)
  (which-key-add-keymap-based-replacements my/insert-keymap
    "s" '("org insert structure template" . org-insert-structure-template))
  ...
  (which-key-add-keymap-based-replacements my/error-keymap
    ...
    "c" '("consult compile error" . consult-compile-error))

  ;; Rings and Regs keybindings
  (defvar-keymap my/rings-and-regs-keymap)
  (which-key-add-keymap-based-replacements my/rings-and-regs-keymap
    "k" '("consult yank from kill ring" . consult-yank-from-kill-ring)
    "j" '("evil collection consult jump list" . evil-collection-consult-jump-list)
    "s" '("consult register store" . consult-register-store)
    "r" '("consult register" . consult-register))
  ...
  (which-key-add-keymap-based-replacements my/buffer-keymap
    ...
    "I" '("consult imenu multi" . consult-imenu-multi))

  ;; Window keybindings
  (defvar-keymap my/window-keymap)
  (which-key-add-keymap-based-replacements my/window-keymap
    "k" '("evil window up" . evil-window-up)
    "j" '("evil window down" . evil-window-down)
    "h" '("evil window left" . evil-window-left)
    "l" '("evil window right" . evil-window-right)
    "c" '("evil window delete" . evil-window-delete)
    "C" '("delete other windows" . delete-other-windows))

  ;; Project keybindings
  (defvar-keymap my/project-keymap)
  (which-key-add-keymap-based-replacements my/project-keymap
    "b" '("projectile switch to buffer" . projectile-switch-to-buffer)
    "/" '("projectile switch project" . projectile-switch-project)
    "s" '("projectile find related file" . projectile-find-related-file))

  ;; File keybindings
  (defvar-keymap my/file-keymap)
  (which-key-add-keymap-based-replacements my/file-keymap
    "p" '("my open personal config dir" . my/open-personal-config-dir)
    "r" '("recentf open files" . recentf-open-files)
  ...
  (which-key-add-keymap-based-replacements my/code-keymap
    "d" '("xref find definitions" . xref-find-definitions))
  ...
  (which-key-add-keymap-based-replacements my/leader-keymap
    ...
    "g" '("magit" . magit)
    ":" '("consult mode command" . consult-mode-command)
    "Q" '("save buffers kill terminal" . save-buffers-kill-terminal)
...
#+begin_src emacs-lisp
  ...
  (defun my/create-lsp-code-map ()
	...
	(which-key-add-keymap-based-replacements my/lsp-code-keymap
	  "p" '("describe thing at point (LSP)" . lsp-describe-thing-at-point)
	  "a" '("code action (LSP)" . lsp-execute-code-action)
	  "=" '("format buffer (LSP)" . lsp-format-buffer)
	  "h" '("document highlight (LSP)" . lsp-document-highlight))

	(when (lsp-feature? "textDocument/definition")
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "d" '("definition (LSP)" . lsp-find-definition)))

	(when (lsp-feature? "textDocument/references")
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "r" '("references (LSP)" . lsp-find-references)))

	(when (lsp-feature? "textDocument/implementation")
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "i" '("implementation (LSP)" . lsp-find-implementation)))

	(when (lsp-feature? "textDocument/declaration")
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "!" '("declaration (LSP)" . lsp-find-declaration)))

	(when (lsp-feature? "textDocument/typeDefinition")
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "t" '("type definition (LSP)" . lsp-find-type-definition)))
	...
	(when (or (lsp-can-execute-command? "java.navigate.resolveTypeHierarchy")
	  ...
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "T" '("type hierarchy (LSP)" . lsp-treemacs-type-hierarchy)))

	(when (and (functionp 'lsp-treemacs-call-hierarchy)
		   (lsp-feature? "textDocument/callHierarchy"))
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "c" '("call hierarchy (LSP)" . lsp-treemacs-call-hierarchy)))

	(when (lsp-feature? "textDocument/rename")
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap
	    "R" '("rename (LSP)" . lsp-rename)))

	;; Session keybindings
	(let ((my/session-lsp-code-keymap (define-keymap)))
	  (which-key-add-keymap-based-replacements my/session-lsp-code-keymap
	    "l" '("workspace logs show (LSP)" . lsp-workspace-logs-show)
	    "d" '("describe session (LSP)" . lsp-describe-session)
	    "s" '("shutdown workspace (LSP)" . lsp-shutdown-workspace))
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap "S" `("Session" . ,my/session-lsp-code-keymap)))
	...
	(let ((my/workspace-folders-lsp-code-keymap (define-keymap)))
	  (which-key-add-keymap-based-replacements my/workspace-folders-lsp-code-keymap
	    "a" '("add workspace folder (LSP)" . lsp-workspace-folders-add)
	    "r" '("remove workspace folder (LSP)" . lsp-workspace-folders-remove)
	    "o" '("open workspace folder (LSP)" . lsp-workspace-folders-open))
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap "F" `("Workspace Folders" . ,my/workspace-folders-lsp-code-keymap)))
	...
	(let ((my/toggle-lsp-features-code-keymap (define-keymap)))
	  (which-key-add-keymap-based-replacements my/toggle-lsp-features-code-keymap
	    "l" '("toggle lens (LSP)" . lsp-lens-mode)
	    "h" '("toggle headerline breadcrumb (LSP)" . lsp-headerline-breadcrumb-mode)
	    "a" '("toggle modeline code actions (LSP)" . lsp-modeline-code-actions-mode)
	    "o" '("toggle modeline diagnostics (LSP)" . lsp-modeline-diagnostics-mode))
	  (which-key-add-keymap-based-replacements my/lsp-code-keymap "T" `("Toggle LSP Features" . ,my/toggle-lsp-features-code-keymap)))
